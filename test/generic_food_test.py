...
class TestGenericFood(unittest.TestCase):
        ...
        def test_generic_food_invalid_expiration_date(self):
                with self.assertRaises(Exception) as context:
                        GenericFood(name="Banana", purchase_date=datetime(2024, 10, 1), expiration_date=datetime(2024, 9, 30), quantity=5)
                self.assertEqual(str(context.exception), "Expiration date must be after purchase date!")

        def test_generic_food_invalid_quantity(self):
                with self.assertRaises(Exception) as context:
                        GenericFood(name="Banana", purchase_date=datetime(2024, 10, 1), expiration_date=datetime(2024, 12, 10), quantity=-1)
